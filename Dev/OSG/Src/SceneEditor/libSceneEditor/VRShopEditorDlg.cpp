...
VRShopEditorDlg::VRShopEditorDlg(QMainWindow *parent) : QMainWindow(parent)	
{
	...
	m_SceneEditor.setQOSGWidgets(m_pQOSGWidgetSceneEditorPose);
	m_ObjectEditor.setQOSGWidgets(m_pQOSGWidgetSceneEditorObject);
...
void VRShopEditorDlg::addImage2Widget(const QString& aqstrProductCode)	{
	...
	if(tQuery.last() == '\0')	{
		...
		if (tQuery.exec(tqry))	{
			...
			std::string strImagePath = "/Images/";
			QImageReader image(strImagePath.c_str()+ name);
...
void VRShopEditorDlg::addTexture2Widget(const QString& aqstrTextureName)	{
	...
	if(tQuery.last() == '\0')	{
		...
		if (tQuery.exec(tqry))	{
			...
			std::string strTexturePath = "../../Resources/Textures/";
			QImageReader image(strTexturePath.c_str() + name);
